...
const createIncome = expressAsyncHandler(async (req, res) => {
  ...
  if (isRecurring && recurrenceInterval) {
    nextOccurrenceDate = getNextOccurrenceDate(date || new Date(), recurrenceInterval);
  }
...
const updateIncome = expressAsyncHandler(async (req, res) => {
  ...
  let nextOccurrenceDate = income.nextOccurrenceDate;
  if (isRecurring !== income.isRecurring || recurrenceInterval !== income.recurrenceInterval) {
    nextOccurrenceDate = getNextOccurrenceDate(date || new Date(), recurrenceInterval);
  }
  ...
  if (previousAmount !== amount) {
    const user = await User.findById(req.user._id);
    user.totalIncome = user.totalIncome - previousAmount + amount;
    user.totalBalance = user.totalBalance - previousAmount + amount;
    user.totalSavings = user.totalBalance - user.totalExpense;
